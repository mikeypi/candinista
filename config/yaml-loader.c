// yaml-loader.c  (single-file refactor)
...
static int st_set_x_index(const char *v, void *obj) {
  SensorParameters *st = (SensorParameters*)obj;
  st->x_index = atoi(v);
  return 1;
}
static int st_set_y_index(const char *v, void *obj) {
  SensorParameters *st = (SensorParameters*)obj;
  st->y_index = atoi(v);
  return 1;
}
...
static int gt_set_x_index(const char *v, void *obj) {
  PanelParameters *gt = (PanelParameters*)obj;
  gt->x_index = atoi(v);
  return 1;
}
static int gt_set_y_index(const char *v, void *obj) {
  PanelParameters *gt = (PanelParameters*)obj;
  gt->y_index = atoi(v);
  return 1;
}
static int gt_set_z_index(const char *v, void *obj) {
  PanelParameters *gt = (PanelParameters*)obj;
  gt->z_index = atoi(v);
  return 1;
}
...
static GHashTable *sensor_handlers(void)
{
  ...
  g_hash_table_insert(ht, "x_index",         (gpointer)(kv_handler*) &(kv_handler){ "x_index", st_set_x_index, NULL });
  g_hash_table_insert(ht, "y_index",         (gpointer)(kv_handler*) &(kv_handler){ "y_index", st_set_y_index, NULL });
...
static GHashTable *panel_handlers(void)
{
  ...
  g_hash_table_insert(ht, "x_index",           (gpointer)(kv_handler*) &(kv_handler){ "x_index", gt_set_x_index, NULL });
  g_hash_table_insert(ht, "y_index",           (gpointer)(kv_handler*) &(kv_handler){ "y_index", gt_set_y_index, NULL });
  g_hash_table_insert(ht, "z_index",           (gpointer)(kv_handler*) &(kv_handler){ "z_index", gt_set_z_index, NULL });
...
static const kv_handler SENSOR_TABLE[] = {
  ...
  {"x_index",         st_set_x_index,         NULL},
  {"y_index",         st_set_y_index,         NULL},
...
static const kv_handler PANEL_TABLE[] = {
  ...
  {"x_index",           gt_set_x_index,           NULL},
  {"y_index",           gt_set_y_index,           NULL},
  {"z_index",           gt_set_z_index,           NULL},
...
void cfg_build_tables (Configuration* cfg) {
  ...
  while (p < cfg->panels + cfg->panel_count) {
    ...
    set_item_in_d3_array (cfg->panel_array,  (uintptr_t*) *p, i, j, k);
    p++;
  }
  ...
  for (i = 0; i < cfg->x_dimension; i++) {
    for (j = 0; j < cfg->y_dimension; j++) {
      set_item_in_d3_array (cfg->active_z_index, (long int)0, i, j, 0);
    }
  }

  int active_can_addresses_count = 0;
  int temp_can_id = 0;

  for (i = 0; i < cfg->sensor_count; i++) {
    if ((0 != cfg -> sensors[i] -> can_id) && (temp_can_id != cfg -> sensors[i] -> can_id)) {
      active_can_addresses_count++;
      temp_can_id = cfg -> sensors[i] -> can_id;
    }
  }

  cfg -> sensor_groups = (sensor_group*) calloc (active_can_addresses_count + 1, sizeof (*cfg -> sensor_groups));
  ...
  while (i < cfg->sensor_count) {
    if (0 != cfg -> sensors[i] -> can_id) {
      cfg -> sensor_groups[k].can_id = cfg -> sensors[i] -> can_id;
      cfg -> sensor_groups[k].first = cfg -> sensors[i];
      cfg -> sensor_groups[k].last = cfg -> sensors[i];

      j = i;
      while ((j < cfg -> sensor_count)
	     && (cfg -> sensor_groups[k].can_id == cfg -> sensors[j] -> can_id)) {
	cfg -> sensor_groups[k].last = cfg -> sensors[j];
	j++;
      }
  ...
  cfg -> panel_groups = (panel_group*) calloc (2 * 3, sizeof (*cfg -> sensor_groups));

  for (i = 0; i < 2; i++) {
    for (j = 0; j < 3; j++) {
      for (k = 0; k < cfg -> panel_count; k++) {
	if ((cfg -> panels[k] -> x_index == i)
	    && (cfg -> panels[k] -> y_index == j)) {
	  if (NULL == cfg -> panel_groups[i * 2 + j].first) {
	    cfg -> panel_groups[i * 2 + j].first = cfg -> panels[k];
	  } else
	    cfg -> panel_groups[i * 2 + j].last = cfg -> panels[k];
	}
      }
    }
  }
...
long cfg_get_active_z  (Configuration* cfg, int i, int j) {
  uintptr_t* ip = get_item_in_d3_array (cfg->active_z_index, i, j, 0);
  return ((long) ip);
}

void cfg_set_active_z (Configuration* cfg, int x_index, int y_index, long value) {
  set_item_in_d3_array (cfg->active_z_index, (uintptr_t*)value, x_index, y_index, 0);
}
